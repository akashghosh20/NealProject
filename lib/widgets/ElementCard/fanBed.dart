...
class FanBed extends StatefulWidget {
  const FanBed({Key? key}) : super(key: key);

  @override
  State<FanBed> createState() => _FanBedState();
}

class _FanBedState extends State<FanBed> {
  bool isFanOn = false;
  DateTime? startTime;
  ...
  void loadElapsedTime() {
    final storedDuration = prefs.getInt('fanBed_elapsed_duration') ?? 0;
    setState(() {
  ...
  Future<void> saveElapsedTime(Duration duration) async {
    await prefs.setInt('fanBed_elapsed_duration', duration.inSeconds);
  }

  Future<void> saveElapsedTaka(double taka) async {
    await prefs.setDouble('fanBed_elapsed_taka', taka);
  }

  Future<void> saveElapsedUnit(double unit) async {
    await prefs.setDouble('fanBed_elapsed_unit', unit);
  }
    ...
    setState(() {
      if (newValue) {
        ...
        if (startTime != null) {
          ...
          saveElapsedUnit(elapsedUnitFanBed);
